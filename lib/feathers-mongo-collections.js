	...
	var dbService = {
		...
		remove: function(id, params, callback) {
		  ...
		  changeDB.dropCollection(id, function(err){
		  	if (err) {
		  		// If the collection wasn't found on this database...
		  		if (err.errmsg === 'ns not found') {
		  			// Return a verbose "collection not found" error.
		  			return callback(id + ' was not found on ' + dbName + '.');
		  		}
		  		// All other errors.
		  		return callback(err.errmsg);
		  	}
				callback(null, {removed:id});
		  });
		},
